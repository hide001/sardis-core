...
task4(){
  ...
  if [[ $totalValidator -gt 0 ]]; then
      echo -e '\ncd /root/sardis-core/' >>/etc/profile
      echo -e '\nbash /root/sardis-core/node-start.sh --validator' >>/etc/profile
  fi

  if [[ $totalRpc -gt 0 ]]; then
      echo -e '\ncd /root/sardis-core/' >>/etc/profile
      echo -e '\nbash /root/sardis-core/node-start.sh --rpc' >>/etc/profile
  fi
...
displayWelcome(){
  ...
  echo -e "${GREEN}
  \t+------------------------------------------------+
  \t+   Sardis node installation Wizard
